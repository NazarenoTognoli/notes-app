...
setTimeout(()=>{
  items.push({ id: 6, content: "Items number 6", title: "Item number 6 title" }); //new item simulation
}, 3000);
...
export class GetItemsService {
  ...
  getItems(): Observable<typeof items> { //Observable<> is a class that shapes return
    // Simulate a server delay of 1 second before returning the data
    return of(items).pipe(delay(100)); //Of creates the observer¿¿¿ //Pipe attaches a function to the return result
